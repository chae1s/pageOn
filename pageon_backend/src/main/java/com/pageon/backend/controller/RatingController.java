...
public class RatingController {
    ...
    @PatchMapping
    public ResponseEntity<Void> updateContentRating(@AuthenticationPrincipal PrincipalUser principalUser, @RequestBody ContentEpisodeRatingRequest request) {
        ContentType contentType = request.getContentType();

        if (contentType == ContentType.WEBNOVEL) {
            ratingService.updateWebnovelRating(principalUser.getId(), request);
        } else if (contentType == ContentType.WEBTOON) {
            ratingService.updateWebtoonRating(principalUser.getId(), request);
        }
