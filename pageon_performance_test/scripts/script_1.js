...
export default function () {
    ...
    const getParams = (tagName) => {
        return {
            headers: {
                'Authorization': `Bearer ${authToken}`,
                'Content-Type': 'application/json',
            },
            tags: { name: tagName } // API 식별을 위한 태그
        };
    };
    ...
    // 전체 요청의 10% 비율로 발생
    if (Math.random() < 0.1) {
        check(http.post(`${BASE_URL}/${content.urlType}/${content.contentId}/interests`, null, getParams('POST_Interests')), { 
            'POST_Interests is 2xx': (r) => r.status >= 200 && r.status < 300 
        });
    }

    // --- [액션 2] 구독 상태 확인 ---
    const subRes = http.get(`${BASE_URL}/${content.urlType}/${content.contentId}/episodes/${episodeId}/subscribe`, getParams('GET_Subscribe_Status'));
    check(subRes, { 'GET_Subscribe_Status is 2xx': (r) => r.status >= 200 && r.status < 300 });
    ...
    if (!isSubscribed) {
        ...
        const buyRes = http.post(
            ...
            null, 
            getParams('POST_Subscribe_Purchase')
        );
        
        check(buyRes, { 'POST_Subscribe_Purchase is 200': (r) => r.status === 200 });    
